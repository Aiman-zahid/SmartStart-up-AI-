...
def default_combined_csv_paths() -> list[str]:
    project_dir = os.path.dirname(__file__)
    return [
        os.path.join(project_dir, "data", "combined_startup_dataset.csv"),
        os.path.join(project_dir, "combined_startup_dataset (1)]n", "combined_startup_dataset.csv"),
    ]


def find_combined_dataset_csv() -> str | None:
    for path in default_combined_csv_paths():
        if os.path.exists(path):
            return path
    return None


def load_existing_combined_dataset() -> tuple[pd.DataFrame, pd.DataFrame] | None:
    """Load a pre-built combined dataset when the Crunchbase zip is unavailable."""
    csv_path = find_combined_dataset_csv()
    if not csv_path:
        return None

    combined = normalize_startup_schema(pd.read_csv(csv_path))
